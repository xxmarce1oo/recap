...
export default function StatsWidget({ reviewCount, distribution }: Props) {
  // Encontrar o valor máximo para a escala logarítmica.
  const maxCountLog = Math.log(Math.max(1, ...Object.values(distribution)) + 1);

  return (
    <div className="bg-gray-800/50 p-4 rounded-lg">
      
      {/* Cabeçalho: Título, linha e contagem */}
      ...
      {/* Histograma */}
      {reviewCount > 0 ? (
        // ✅ ALTERAÇÃO AQUI: de 'items-center' para 'items-end'
        <div className="flex items-end gap-3">
          ...
          <div className="flex items-end justify-between w-full h-20 gap-px">
            {RATING_LEVELS.map(rating => {
              ...
              // Calcula a altura usando uma escala logarítmica para um visual mais equilibrado.
